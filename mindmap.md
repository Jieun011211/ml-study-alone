```mermaid
  ...
  root((ML alone))
    ...
    순차 데이터와 순환 신경망
      순차 데이터
        시계열 데이터와 같이 순서가 있는 데이터
      순환 신경망
        순차 데이터에 맞는 인공 신경망의 한 종류
          셀_cell: 순환층
            여러 개의 뉴런으로 구성
            은닉상태: 셀의 출력
      IMDB 리뷰 분류하기
        말뭉치
          자연어 처리에서 사용하는 텍스트 데이터의 모음_훈련 데이터셋
        토큰
          텍스트에서 공백으로 구분되는 문자열
      원-핫 인코딩
      ...
      단어 임베딩
        정수로 변환된 토큰을 비교적 작은 크기의 실수 밀집 벡터로 변환
          단어 사이의 관계를 표현할 수 있어서 자연어 처리에 좋은 성능
    LSTM 과 GRU 셀
      LSTM_4개의 셀
        타입스텝이 긴 데이터를 효과적으로 학습하는 순환층
          입력게이트
          삭제게이트
          출력게이트
        셀 상태 출력
